    ...
    export default {
        ...
        created() {
            ...
            this.$http.get(Api.GET_FAMILY_CONCAT_INFO + params)
                ...
                .then(res => {
                    if (res.datas.JTDZ) {
                        this.address = res.datas.JTDZ;
                    }
                    if (res.datas.JTYB) {
                        this.zip = res.datas.JTYB;
                    }
                    if (res.datas.JTDH) {
                        this.phone = res.datas.JTDH;
                    }
                    Indicator.close();
                });
        },
        methods: {
            complete: function () {
                if (this.address.length === 0) {
                    ...
                    Indicator.open();
                    let params = '?IDENTITY_ID=' + Api.TEST_ID + '&JTDZ=' + this.address + '&JTYB=' + this.zip + '&JTDH=' + this.phone;
                    this.$http.get(Api.MODIFY_FAMILY_CONCAT_INFO + params)
                        .then(res => {
                            return res.json();
                        })
                        .then(res => {
